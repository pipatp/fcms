...
<script>
    ...
    function addWorklistRow($tableBody, item) {
        ...
        $("<td>", { "class": "fit-content" }).text(item.TwlStrDtm).appendTo($row);
        $("<td>", { "class": "fit-content" }).text(item.TwlEndDtm).appendTo($row);
        $("<td>", { "class": "content" }).text(item.TwlSchDtl).appendTo($row);
        
        var $deleteButton = $('<div class="btn btn-danger btn-sm">ลบ</div>');
        $deleteButton.attr("data-appointment-seq", item.TwlTapSeq);
        $deleteButton.attr("data-item-seq", item.TwlSeqNum);
        
        $deleteButton.click(deleteWorklistRow);
        
        var $notApprove = $('<div class="label label-warning">ยังไม่อนุมัติ</div>');
        var $Approved = $('<div class="label label-success">อนุมัติแล้ว</div>');
     //   $approveButton.click(approveWorklistRow);
     //   $("<td>", { "class": "fit-content" }).text(item.TwlApvStt === "Y" ? "sda" : "wwww").appendTo($row);
        $("<td>", { "class": "fit-content" }).html(item.TwlApvStt === "N" ? $notApprove : $Approved).appendTo($row);
    ...
    function deleteWorklistRow() {
        ...
        $.post("deleteTeamWorklistItem", JSON.stringify(deleteRequest)).done(function() {
    ...
    function loadTeamSchedule($datePicker) {
        var date = getDateFromDatePicker($datePicker, "yymmdd");
        
        $.get("getTeamSchedule/" + date).done(function(result) {
            var schedule = jQuery.parseJSON(result);
            
            $("#appointment-detail .stretch").text("");
            
            if (schedule.appointment.TapAppDtl) {
                $("#appointment-detail .stretch").text(schedule.appointment.TapAppDtl);
            }
            
            $("#addDialog").attr("data-appointment-seq", schedule.appointment.TapSeqNum);
    ...
    function addWorklistItem() {
        ...
        $.post("addTeamSchedule", JSON.stringify(addRequest)).done(function() {
    ...
    $("#appointment-detail").focusout(function() {
        ...
        if ($commentBox.val() === currentComment) {
            ...
            $.post("updateTeamScheduleDetail", JSON.stringify(updateRequest)).done(function() {
                convertToDiv($section);
            }).fail(function() {
                alert("ไม่สามารถลบข้อมูลได้ โปรดลองอีกครั้งหนึ่ง");
            });
